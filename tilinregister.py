...
def register(message):
    if str(message.from_user.id) in registered_users:
        ...
        bot.send_message(message.chat.id, 'Registro exitoso. Ahora puedes usar el comando /tilinrefe.', parse_mode='HTML')
...
def handle_reference_command(message):
    ...
    if str(user_id) not in registered_users:
        bot.send_message(message.chat.id, 'Por favor, regístrate para poder usar este comando. Utiliza /register', parse_mode='HTML')
        return
...
def handle_reference(message):
    ...
    if user_id in banned_users:
        bot.reply_to(message, "Lo siento, no tienes permitido usar el bot. Estás baneado. Si crees que fue un error, contacta a @BannedEnd para que te quite el ban.", parse_mode='html')
        return
